        ...
        for run in flowcell.runs.all():
            try:
                ...
                # If there isn't a run summary object create one
                    ...
                    logger.info(
                        "Flowcell id {} - ELSE No objects in database creating new ones".format(
                            flowcell.id
                        )
                    )
            ...
            # Except No run summary found. One of these code blocks is unnecessary
                ...
                logger.info(
                    "Flowcell id {} - EXCEPTION No objects in database creating new ones".format(
                        flowcell.id
                    )
                )
                run_summary = RunSummary.objects.create(
            ...
            if not run_summary.running:
                ...
                if reads.count() > 0:
                    ...
                    if run_summary.read_count != None:
                        logger.info(
                            "Flowcell id {} - Previous results seen for run summary".format(
                                flowcell.id
                            )
                        )
                        # Sum the read counts
                        ...
                        run_summary.max_read_length = max(
                        ...
                        run_summary.min_read_length = min(
                        ...
                        # gets turnt up
                        logger.info(
                            "Flowcell id {} - NO Previous results seen for run summary, setting first values".format(
                                flowcell.id
                            )
                        )
                        run_summary.read_count = result["sequence_length__count"]
