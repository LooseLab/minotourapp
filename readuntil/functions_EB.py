...
def initialise_priors_rory(reference, genotypes, theta=0.009, r=0.11, model="DNAhaplo"):
    ...
    for g in genotypes[:len(genotypes) - 1]:
        # append an array to priors for wach of the four base types, where if the reference base is the same as the for loop base
        # the element is the same as 1 - theta * 1 = r, if it's not the same, the element is the same as theta divided by 5-2
        priors.append(np.where((query_array == g), (1.0 - (theta * (1.0 + r))), theta / (len(genotypes) - 2)))
    # add one final array to the priors list where there is as many elements as reference bases and the element value is theta * r
    priors.append(np.full(query_array.size, theta * r))
    # turn the priors list into an numpy array
    priors = np.array(priors)
    # return the priors array transformed so it's as many rows as the reference with 5 element in each, one from each array above
    return priors.T
...
def position_benefit_rory(posterior_array, phi):
    ...
    entropies = -np.sum(posterior_array * np.log(posterior_array), axis=1)
    ...
    for i in range(len(phi)):
        ...
        observation_probabilities = np.concatenate(
            (observation_probabilities, np.sum(phi[:, i] * posterior_array, axis=1)))
        # New posteriors is the same as the observation probabilities, but each element in a row is normalised by the sum of the row
        # So E is the posterior array mulitplied by a column of phi
        e = phi[:, i] * posterior_array
        # Now divide each row of e by it's sum, should be one for each posterior
        e = e / e.sum(axis=1, keepdims=True)
        # Append the normalise rows to this list
        newPosteriors.append(e)

    # Reshape the probabilitiesto the len of phi columns and number of posteriors rows and transfrom it
    observation_probabilities = np.resize(observation_probabilities, (len(phi), len(posterior_array))).T

    newEntropies = list()
    # For each phi, so 5
    for i in range(len(phi)):
        # Create a new Entropies list that contains the negative sum value of new posteriors * observation_probabilites,
        # ravelled to 1d, then shaped into a 2d array (so the multiplication of the two arrays is respective), multiplied by the log of the new log of the posteriors
        newEntropies.append(-np.sum(
            newPosteriors[i] * observation_probabilities[:, i, None].ravel()[:, np.newaxis] * np.log(newPosteriors[i]),
            axis=1))

    # Sum the columns of the newEntropies array, returns 1d array
    newEntropies = np.array(newEntropies).sum(axis=0)

    # Return the difference between the two entropies
    return entropies - newEntropies


def update_posteriors_notts(rory_test_posteriors, rory_test_counts, rory_testphi):
    # Check your phis aren't too high
    # rory_test_posteriors array is a nested array the length of the reference, there are 5 elements in each nest, one for each base plus deletions
    # rory_test_counts is the counts structured data array, as many elements as there are reference bases, 9 elements in each array
    ...
    d = np.resize(np.repeat(rory_testphi[None, ...], 5, axis=0), (reference_length * 5, 5))
    # Expand rory test counts in 2d array, with 5 copies of each of each test count element

    # Do in place exponentials, memory efficient
    d **= np.repeat(np.array(rory_test_counts.tolist()), rory_test_posteriors.shape[1], axis=0)
    ...
    # Cumulative product the row
    results = np.cumprod(arr, axis=1)
    # Ravel the last column of the 2d results array
    results = results[:, -1:].ravel()

    # Np resize the results to be a 2d array of 5 elements, with as many rows as you have reference positions, so correct shape as beginning posteriors
    results = np.resize(results, (reference_length, 5))
    print(results)
    # Normalise the posteriors against the sum of the row
    posteriors = results / results.sum(axis=1)[:, None]
...
def calculate_read_benefits_fixed_read_length_rory(benefits_rory, lam, m):
    ...
    benefits_rory_test = np.insert(benefits_rory_test, 0, partialS_rory, axis=0)
...
def calculate_costs_fixed_read_length_rory(N, rho, m, lam):
    ...
    read_costs_rory_reverse = read_costs_rory_forward[::-1]

    return read_costs_rory_forward, read_costs_rory_reverse


def calculate_scores_rory(read_benefits_rory_forward, read_benefits_rory_reverse, m_benefits_rory_forward,
                          m_benefits_rory_reverse, read_costs_rory_forward, read_costs_rory_reverse):
    ...
    scores_F[~index] = (read_benefits_rory_forward[~index] - m_benefits_rory_forward[~index]) / read_costs_rory_forward[
        ~index]
    ...
    scores_R[~index] = (read_benefits_rory_reverse[~index] - m_benefits_rory_reverse[~index]) / read_costs_rory_reverse[
        ~index]
...
def find_strategy_uniform(
    ...
    for pos in range(len(scoresF)):
        totScores.append((scoresF[pos], 1, pos))
        acceptF.append(0)
    ...
    StrategyCost = (alpha + m + rho) * (len(scoresF) + len(scoresR)) - m * (
            m - 1
    )  # the last bit is to account for positions very close to the end, for which lamda_i<m . Maybe I should assume 0 benefit for these?
    # 3 - find the best strategy
    for score in totScores:
        if score[1] == 1:  # if it's a forward read
            newB = StrategyBenefit + readBenefitsF[score[2]] - mBenefitsF[score[2]]
            ...
            newB = StrategyBenefit + readBenefitsR[score[2]] - mBenefitsR[score[2]]
