...
from reads.models import FastqRead, FastqReadType
...
def run_minimap2_alignment(job_master_id):
    ...
    logger.info(
        "Flowcell id: {} - Running minimap2 on flowcell {}".format(
            flowcell.id, flowcell.name
        )
    )
    logger.info("Flowcell id: {} - job_master_id {}".format(flowcell.id, job_master.id))
    logger.info(
        "Flowcell id: {} - reference {}".format(flowcell.id, job_master.reference.name)
    )
    logger.info(
        "Flowcell id: {} - last_read {}".format(flowcell.id, job_master.last_read)
    )
    logger.info(
        "Flowcell id: {} - read_count {}".format(flowcell.id, job_master.read_count)
    )
    logger.info(
