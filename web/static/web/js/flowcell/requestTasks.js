function loadTasksForm () {
  /** TODO rewrite, move to controller, write like drop down for minimap2
     * @function
  ...
  $.get(url_task_type_list, function (dataObj, statusText, xhr) {
    ...
    const alreadyPresentJobs = [...$(`#id_job_type`)[0].children]
    ...
    alreadyPresentJobs.forEach(function (presentJob) {
      ...
      if (data.some(e => e.description === presentJob.textContent)) {
        // get the index,
        const index = data.map(function (x) { return x.description }).indexOf(presentJob.textContent)
  ...
  $.get(url_reference_list, data => {
    ...
    const alreadyPresentRefs = [...$(`#id_reference`)[0].children]
    ...
    alreadyPresentRefs.forEach(function (presentRef) {
      ...
      if (references.some(e => e.name === presentRef.textContent)) {
        // get the index,
        const index = references.map(function (x) { return x.name }).indexOf(presentRef.textContent)
        // remove it from the freshly fetched chromosomes
