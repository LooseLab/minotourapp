class FlowcellNotificationController {
  ...
  submitNotificationChoices (event, data) {
    ...
    const contigSelect = $(`#contig`)
    const barcSelect = $(`#notification_barcodes`)
    const selectedAutoNotifications = multiInputAuto.getValues()
    const justElements = [refSelect, contigSelect, barcSelect]

    const self = this
    // loop through the select boxes
    if (refSelect[0] !== undefined) {
      const refSelectOptions = refSelect[0].selectedOptions
      const contigSelectOptions = contigSelect[0].selectedOptions
      const barcSelectOptions = barcSelect[0].selectedOptions
      const selects = [[refSelectOptions, `reference`], [contigSelectOptions, `chromosome`], [barcSelectOptions, `barcodes`]]
      for (const property in Object.keys($(`input`, data))) {
        // Get the input element
        const input = $(`input`, data)[property]
        console.log(input)
        if (input === undefined) {
          continue
        }
      ...
      if (`coverage` in this._conditions) {
        selects.forEach(function (selected) {
          ...
          if (key === `barcodes`) {
            ...
            const chosen_barcodes = multiInput.getValues()
    ...
    axiosInstance.post(`/api/v1/communication/messages/conditions`, {
  ...
  getReferencesForDataList () {
    ...
    axiosInstance.get(`/api/v1/communication/${self._flowcellId}/chromosomecoverage`, {
      params: { names: true }
    }).then(function (names) {
  ...
  addHandlerToSelect () {
    ...
    [$(`#notification_barcodes`), $(`#contig`)].forEach(function (a) {
      a.on(`click`, function () {
        if ($(this).find(`:selected`).text() == `Select All`) {
          if ($(this).attr(`data-select`) == `false`) { $(this).attr(`data-select`, `true`).find(`option`).prop(`selected`, true) } else { $(this).attr(`data-select`, `false`).find(`option`).prop(`selected`, false) }
        }
      })
    })

    $(`#reference`).on(`change`, function (e) {
      const notificationContigs = $(`#contig`)
      const bar = $(`#notification_barcodes`)
      ...
      const referenceSelected = new Set()
      ...
      Object.entries(optionSelected).forEach(([key, value]) => {
        if (value.innerHTML !== undefined) {
          const a = self._contigsObj[optionSelected[0].innerHTML]
          console.log(a)
    ...
    $(`#contig`).on(`change`, function (e) {
      e.preventDefault()
      const notificationBarcodes = $(`#notification_barcodes`)
      ...
      const contigsSelected = new Set()
      // for each of our selected contigs
      console.log(Object.entries(optionSelected))
      for (let i = 0; i < optionSelected.length; i++) {
        const key = i
        const value = optionSelected[i]
