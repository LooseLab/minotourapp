    ...
    for flowcell in Flowcell.objects.filter(is_active=True):
        ...
        for run in flowcell.runs.all():
            ...
            if not run_summary.running:
                ...
                run_summary.save()
                # Get up to 50000 reads that have appeared since the last read that we saw (The id of which is stored)
                reads = FastqRead.objects.filter(run=run).filter(id__gt=int(run_summary.last_read))[:50000]
                # If there are any new reads TODO test this, should work but may count previous reads
                if reads.count() > 0:
                    logger.info(
